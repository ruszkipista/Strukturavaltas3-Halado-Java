...
Ellenőrizd, hogy fut-e a MariaDB szerver a lokális számítógépen (`localhost`)!
Jelentkezz be `root` felhasználóval HeidiSQL kliens használatával, és futtasd le a következő parancsot:

```sql
use mysql;
create schema if not exists jpatraining default character set utf8 collate utf8_hungarian_ci;

create user 'training'@'localhost' identified by 'training';
grant all on *.* to 'training'@'localhost';
```
Ekkor létrejön az `jpatraining` séma, és a `training` felhasználó, ugyanezzel a jelszóval.
...
Ehhez hozz létre egy `Coordinate` DTO-t ([Data Transfer Object](https://www.baeldung.com/java-dto-pattern)), melynek egy `latitude` és `longitude`
attribútuma van!
